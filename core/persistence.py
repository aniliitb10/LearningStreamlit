...
class Persistence:
    ...
    def _persist_new_or_edited(self, state: Operation) -> Optional[requests.Response]:
        ...
        return self._request_handler_class.handle_post(url=self._config.get_movies_end_point(EndPoint.Post),
    ...
    def _persist_deleted(self) -> Optional[requests.Response]:
        ...
        return self._request_handler_class.handle_delete(url=self._config.get_movies_end_point(EndPoint.Delete),
                                                         json_data=json.dumps(deleted_ids),
                                                         headers=self.common_headers)

    @staticmethod
    def get_data(url: Optional[str] = None, model_class: Type[Model] = None) -> pd.DataFrame:
        ...
        if not url:
            url = config.get_movies_end_point(EndPoint.Get)

        if not model_class:
            model_class: Type[Model] = config.get_model_class()

        response: requests.Response = config.get_request_handler_class().handle_get(url=url)
        if response.status_code != 200:
            raise ConnectionError(response)

        df: pd.DataFrame = pd.read_json(StringIO(response.text))
